	...
	<div id="header">
		...
		<div id="navigation">
			<ul>
				...
				<li><a style="color: #00c73c;" href="qnaBoard">Q&A 게시판</a></li>
			</ul>
		</div>
	</div>
	<!-- /#header -->
		...
		<div class="body">
			<div id="main">
				...
				<c:if test="${sessionScope.member.member_id != null }">
					<button style="float: right; margin-bottom: 20px;"
						class="list-button" onclick="location.href='/qnaInsert'">글쓰기</button>
				</c:if>
			</div>
			...
			<div class="col-xs-12">
				...
				<table width="960" align="left">
					...
					<c:forEach var="qna" items="${qnaBoard}">
						<tr style="cursor: pointer" class="move"
							onclick="location.href='/qnaDetail/${qna.bseq}'">
							<td>${qna.bseq}</td>
							<td style="color: #73b9ff">${qna.catType}</td>
							<td>${qna.title}</td>
							<td>${qna.nick_name}</td>
				...
				<div class="pageInfo_area">
					<ul class="pageInfo">
						<c:if test="${page.prev}">
							<li><a
								href='<c:url value="/qnaBoard?page=${page.startPage-1}"/>'>이전</a></li>
						</c:if>
						<c:forEach begin="${page.startPage}" end="${page.endPage}"
							var="num">
							<li class="${page.cri.page == num ? "active":"" }"><a
								class="num" href='<c:url value="/qnaBoard?page=${num}"/>'>${num}</a></li>
						</c:forEach>
						<c:if test="${page.next && page.endPage>0}">
							<li><a
								href='<c:url value="/qnaBoard?page=${page.endPage+1}"/>'>다음</a></li>
						</c:if>
					</ul>
				</div>
			</div>
