	...
	<div id="header">
		...
		<div id="navigation">
			<ul>
				...
				<li><a style="color: #00c73c;" href="companyBoard">기업 게시판</a></li>
			...
			<div id="main">
				...
				<c:if test="${sessionScope.member.member_id != null }">
					<button style="float: right; margin-bottom: 20px;"
						class="list-button" onclick="location.href='/companyInsert'">글쓰기</button>
				</c:if>
			</div>
			...
			<div class="col-xs-12">
				...
				<table width="960" align="left">
					...
					<c:forEach var="comp" items="${companyBoard}">
						<tr style="cursor: pointer" class="move"
							...
							<td><fmt:formatDate value="${comp.regdate}"
				...
				<div class="pageInfo_area">
					<ul class="pageInfo">
						<c:if test="${page.prev}">
							<li><a
								href='<c:url value="/companyBoard?page=${page.startPage-1}"/>'>이전</a></li>
						</c:if>
						<c:forEach begin="${page.startPage}" end="${page.endPage}"
							var="num">
							<li class="${page.cri.page == num ? "active":"" }"><a
								class="num" href='<c:url value="/companyBoard?page=${num}"/>'>${num}</a></li>
						</c:forEach>
						<c:if test="${page.next && page.endPage>0}">
							<li><a
								href='<c:url value="/companyBoard?page=${page.endPage+1}"/>'>다음</a></li>
						</c:if>
					</ul>
				</div>
			</div>
