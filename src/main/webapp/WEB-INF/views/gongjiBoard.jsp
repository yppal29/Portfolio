	...
	<div id="header">
		...
		<div id="navigation">
			<ul>
				...
				<li><a style="color: #00c73c;" href="gongjiBoard">공지사항</a></li>
			...
			<div class="col-xs-12">
				...
				<table width="960" align="left">
					...
					<c:forEach var="gongji" items="${gongjiBoard}">
						<tr style="cursor: pointer" class="move"
							...
							<td>${gongji.bseq}</td>
							<td style="color: #73b9ff">${gongji.catType}</td>
							<td>${gongji.title}</td>
							<td>${gongji.nick_name}</td>
							<td><fmt:formatDate value="${gongji.regdate}"
				...
				<div class="pageInfo_area">
					<ul class="pageInfo">
						<c:if test="${page.prev}">
							<li><a
								href='<c:url value="/gongjiBoard?page=${page.startPage-1}"/>'>이전</a></li>
						</c:if>
						<c:forEach begin="${page.startPage}" end="${page.endPage}"
							var="num">
							<li class="${page.cri.page == num ? "active":"" }"><a
								class="num" href='<c:url value="/gongjiBoard?page=${num}"/>'>${num}</a></li>
						</c:forEach>
						<c:if test="${page.next && page.endPage>0}">
							<li><a
								href='<c:url value="/gongjiBoard?page=${page.endPage+1}"/>'>다음</a></li>
						</c:if>
					</ul>
				</div>
			</div>
